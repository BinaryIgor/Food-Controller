...
class MealDetailsDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val binding: DialogItemsBinding = DataBindingUtil.inflate(
            LayoutInflater.from(this.context),
            R.layout.dialog_items,
            null,
            false
        )
        this.viewModel.meal(LifecycleCallback(this) { r ->
            if (r.isSuccess()) {
                ...
                binding.items.addItemDecoration(LinearOffsetDecoration(this.context as Context, 4))
