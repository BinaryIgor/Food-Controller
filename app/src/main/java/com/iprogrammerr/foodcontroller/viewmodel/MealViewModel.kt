...
class MealViewModel(private val asynchronous: Asynchronous, private var id: Long, private val meals: Meals) :
    ViewModel() {

    constructor(asynchronous: Asynchronous, meals: Meals) : this(asynchronous, -1, meals)
    ...
    companion object {
        fun factory(id: Long) = object : ViewModelProvider.NewInstanceFactory() {
            ...
            override fun <T : ViewModel?> create(clazz: Class<T>): T = when {
                clazz.isAssignableFrom(MealViewModel::class.java) ->
                    MealViewModel(id) as T
                else -> throw Exception("$clazz is not a ${MealViewModel::class.java.simpleName}")
            }
    ...
    fun create(time: Long, dayId: Long, callback: Callback<Long>) {
        this.asynchronous.execute({
            this.id = this.meals.create(dayId, time)
            this.meal.unstick()
            this.id
        }, callback)
    }
